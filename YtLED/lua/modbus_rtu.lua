...
--是否大端
local crc_msb = false
function modbus_rtu.set_msb(msb)
  crc_msb = msb
end
...
function modbus_rtu.crc16(data)
  ...
  if isDebug then
    log.info("crc16end1 = ", crc16_data)
  end

  if crc_msb then --切换大端
    crc16_data = utils.swap_end(crc16_data, 16)
    if isDebug then
      log.info("crc16end2 = ", crc16_data)
    end
  end
